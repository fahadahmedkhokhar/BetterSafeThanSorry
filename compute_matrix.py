...
folder_path = './results/STL10'  # Replace this with the path to your folder
...
for csv_file in csv_files:
    ...
    probability_criteria_2 = 0.7

    # Split the string based on the underscore character
    split_values = file_name.split('_')

    # Extract 'ResNet' and 'AlexNet'
    binary_clf = split_values[2]
    multiclass_clf = split_values[3].split('.')[0]  # Removing the '.csv' extension
    ...
    with open('output.txt', 'a') as f:
        ...
        print("Misclassification:", simple_miss)
        ...
        print("===========================")
        filtered_op = df[df['Probabilities'] >= probability_criteria]
        ...
        df_phi = df[df['Probabilities']< probability_criteria]
        ...
        filtered_sw = df[(df['Predicted Label'] == 1) & (df['Probabilities'] >= probability_criteria)]
        ...
        df_phi = df[(df['Predicted Label'] == 0) | (df['Probabilities'] < probability_criteria)]
        ...
        print("===========================")
        filtered_op2 = df[df['Probabilities'] >= probability_criteria_2]
        ...
        df_phi = df[df['Probabilities'] < probability_criteria_2]
        op_phi = df_phi['Predicted Label'].count() / length
        print("Phi: ", op_phi)
        ...
        filtered_sw2 = df[(df['Predicted Label'] == 1) & (df['Probabilities'] >= probability_criteria_2)]
        ...
        df_phi = df[(df['Predicted Label'] == 0) | (df['Probabilities'] < probability_criteria_2)]
        sw_phi = df_phi['Predicted Label'].count() / length
        print("Phi: ", sw_phi)
