...
class ImageClassifier(Model):
    ...
    def predict_proba(self, data_loader):
        ...
        paths_list = []  # Initialize a list to store file paths
        max_probabilities = []  # Initialize a list to store maximum probabilities
        with torch.no_grad():
            for data, paths in tqdm.tqdm(data_loader, total=len(data_loader)):
                if len(data) == 2:
                    images, labels = data
                    images, labels = images.to(self.device), labels.to(self.device)
                else:
                    images, labels = data.to(self.device), paths.to(self.device)
        ...
        self.predictions = torch.cat(predictions, dim=0)  # Convert the list of predictions to a tensor
        self.labels = torch.cat(labels_list, dim=0)  # Convert the list of labels to a tensor
